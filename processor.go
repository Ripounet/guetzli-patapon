...
func DownsampleImage(OutputImage* img) {
...
func TryQuantMatrix(jpg *JPEGData_in,
                    target_mul float32,
                    q [][kDCTBlockSize]int,
...
func SelectQuantMatrix(jpg *JPEGData_in, downsample bool,
...
func SelectFrequencyMasking(jpg *JPEGData, OutputImage* img,
                                       byte comp_mask,
                                       double target_mul,
                                       bool stop_early) {
  width := img.width();
  ...
  for direction := range []int{1, -1} {
    for {
      ...
      if (first_up_iter) {
        ...
        it := std::partition_point(global_order.begin(), global_order.end(),
                                       [=](const std::pair<int, float>& a) {
                                         return a.second < limit; });
        min_coeffs_to_change = std_max(min_coeffs_to_change,
...
func Processor::ProcessJpegData(params *Params, jpg_in *JPEGData,
...
func Process(params *Params, stats *ProcessStats,
             const string& data,
             string* jpg_out) bool {
  var jpg JPEGData
...
func Process(const Params& params, ProcessStats* stats,
             const []byte& rgb, int w, int h,
             string* jpg_out) bool {
  JPEGData jpg;
