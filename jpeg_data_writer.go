...
func EncodeDQT(const std::vector<JPEGQuantTable>& quant, out JPEGOutput) bool {
  int marker_len = 2;
...
void BuildACHistograms(jpg *JPEGData, JpegHistogram* histo) {
  for (size_t i = 0; i < jpg.components.size(); ++i) {
...
func BuildAndEncodeHuffmanCodes(jpg *JPEGData, out JPEGOutput) (ok bool, dc_huff_tables, ac_huff_tables []HuffmanCodeTable) {
  const int ncomps = len(jpg.components);
